function Present_Random_VerticalMotion_untilFlight(Repeat,VerticalDuration,Interval)

%Written on 2012/8/23 based on Present_Random_VerticalMotion.
%
%Just show 4Hz motion up or down (random order) for 1 second then stop for
%1 sec for Repeat (default 5) times. After the flight is induced, wait Gap
%seconds then show visual motion again.
%
...
SequenceIndex_V=zeros(Repeat+1,2);
SequenceIndex_V(1,:)=[1 2];%First one has to go to the positive direction so it will trigger the airpuff counter

for n=1:Repeat
    SequenceIndex_V(n+1,:)=randperm(2);%Corresponds to 1hz, 2hz, 4hz goin up and down
    for nn=1:2
...
OutPutFile=strcat('Present_Random_VerticalMotion_untilFlight',datestr(TimeMatrix(1,:),'yyyymmddTHHMMSS'));
