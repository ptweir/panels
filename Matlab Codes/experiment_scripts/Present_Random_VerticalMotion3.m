function Present_Random_VerticalMotion3(Repeat,Duration1,Duration2,Duration4,Interval,RepeatB,Gap,DurationB)

%Written on 2012/08/28.
%Same as version 2, except now show 1 Hz motion going up during the first 4
%seconds of Gap so that we can test if having motion during the initiation
%of flight has any effect on the OCT neuron.
%
...
Panel_com('send_gain_bias',[16,0,0,0]);
Panel_com('start')
pause(4);
Panel_com('stop')
...
OutPutFile=strcat('Present_Random_VerticalMotion3',datestr(TimeMatrix(1,:),'yyyymmddTHHMMSS'));
