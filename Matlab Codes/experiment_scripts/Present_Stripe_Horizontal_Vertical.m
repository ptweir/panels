function Present_Stripe_Horizontal_Vertical(Repeat,StripeDuration,HorizontalDuration,VerticalDuration)
...
for n=1:Repeat
    SequenceIndex_H(n,:)=randperm(4);%Corresponds to 1 and 4 hz to the right and to the left.
    SequenceIndex_V(n,:)=randperm(4);
    for nn=1:4
        ...
        %Move the stripe.
        TimeMatrix((n-1)*32+(nn-1)*8+1,:)=clock;
        Panel_com('start')
        pause(StripeDuration)
        Panel_com('stop')
        TimeMatrix((n-1)*32+(nn-1)*8+2,:)=clock;
        ...
        TimeMatrix((n-1)*32+(nn-1)*8+3,:)=clock;
        Panel_com('start')
        pause(HorizontalDuration)
        Panel_com('stop')
        TimeMatrix((n-1)*32+(nn-1)*8+4,:)=clock;
        ...
        %Move the stripe.
        TimeMatrix((n-1)*32+(nn-1)*8+5,:)=clock;
        Panel_com('start')
        pause(StripeDuration)
        Panel_com('stop')
        TimeMatrix((n-1)*32+(nn-1)*8+6,:)=clock;
        ...
        TimeMatrix((n-1)*32+(nn-1)*8+7,:)=clock;
        Panel_com('start')
        pause(VerticalDuration)
        Panel_com('stop')
        TimeMatrix((n-1)*32+(nn-1)*8+8,:)=clock;
    end
end
...
%Move the stripe.
TimeMatrix((n-1)*32+(nn-1)*8+9,:)=clock;
Panel_com('start')
pause(StripeDuration)
Panel_com('stop')
TimeMatrix((n-1)*32+(nn-1)*8+10,:)=clock;


OutPutFile=strcat('Present_Stripe_Horizontal_Vertical',datestr(TimeMatrix(1,:),'yyyymmddTHHMMSS'));
